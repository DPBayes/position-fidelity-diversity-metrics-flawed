...
# - comment on incorrect authenticity calculation
# - disable authenticity calculation
...
def compute_alpha_precision(real_data, synthetic_data, emb_center):
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", "UserWarning: To copy construct from a tensor")
    ...
    # This authenticity calculation is not correct
    # 1. real_to_synth is an array with the same length as the real data, and 
    #    so is authen, so the mean is counting real datapoints that meet a 
    #    condition, not synthetic datapoints
    # 2. real_to_synth_args is an array of indices into the synthetic data, but 
    #    is used to index real data distance array
    # 3. if the synthetic dataset is bigger than the real data, it is possible
    #    that real_to_synth_args contains an index that is larger than the
    #    real dataset size, which causes an IndexError
    # See which one is bigger
    # authen = real_to_real[real_to_synth_args] < real_to_synth
    # authenticity = np.mean(authen.numpy())

    # Disable authenticity calculation for now
    authenticity = None
